...
public class LoginTest extends BaseTest {
	...
	LoginPage loginPage;
	   ...
	   @BeforeMethod
		...
		//   log.info("Opening locators page before each test");
	...
	public void loginWrongCredentials() {
		...
		loginPage.clickSignIn();
		Assert.assertEquals(loginPage.getErrorMessage(), "* Incorrect username or password");
		Assert.assertEquals(driver.getCurrentUrl(),"https://rahulshettyacademy.com/locatorspractice/", "User is not on login page after failed login");
	}
	...
	Assert.assertEquals(driver.getCurrentUrl(), "https://rahulshettyacademy.com/locatorspractice/", "User is not o login page after click on forgot password");
	...
	public void resetPassword() {
		...
		Assert.assertEquals(loginPage.getResetMessage(), "Please use temporary password 'rahulshettyacademy' to Login.", "Message is not the same");
	  ...
	  public void checkboxIsPresent() {
		  ...
		  loginPage.isCheckboxRememberMePresent();
		  loginPage.isCheckboxAgreePresent();
	  ...
	  public void successfullLogin() {
		  String username = "Miller442";
		    ...
		    // Novi assert za username u <h2> tagu
	  ...
	  public void logoutFunctionality() {
		  ...
		  Assert.assertTrue(loginPage.isUsernameDisplayed(), "We are not back to login page, because there is no username field displayed");
		  Assert.assertTrue(loginPage.isPasswordDisplayed(), "We are not back to login page, because there is no PASSWORD field displayed");
	  }
